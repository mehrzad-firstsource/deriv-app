...
export default class TicksService {
    ...
    observe() {
        // deriv_api updated
        this.api.onMessage().subscribe(r => {
            console.log(r.data.msg_type);
            if (r.data.msg_type === 'tick') {
                const {
                    tick,
                    tick: { symbol, id },
                } = r.data;
